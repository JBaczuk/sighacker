...
// TODO:
/* Sign to Contract Verification */
static void verifys2c_usage(const char *name) {
    fprintf(stderr, "  %s verifys2c <public key> <signature> <message> <original nonce> \n", name);
}

static int verifys2c_command(int argc, char *argv[]) {
    unsigned char msg[32];
    unsigned char pkbin[33];
    unsigned char sigbin[72];
    ...
    /* Parse nonce */
    if (strlen(argv[5]) != 64 || !hex2bin(argv[5], nonce)) {
        fprintf(stderr, "nonce « %s » should be a 64-character hex string\n", argv[5]);
        return 0;
    }

    /* Verify */
    if (secp256k1_ecdsa_verify_nonce(ctx, &sig, msg, &pk, nonce)) {
        puts("Valid signature and commitment.");
        secp256k1_context_destroy(ctx);
        return 1;
    } else {
        puts("Invalid signature or commitment.");
        secp256k1_context_destroy(ctx);
        return 0;
    }
}
...
cli_command COMMANDS[] = {
    ...
    { "verifys2c", 5, verifys2c_usage, verifys2c_command }
};
